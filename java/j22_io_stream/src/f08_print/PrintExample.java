...
public class PrintExample {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// PrintStream (byte기반 가능)
			PrintStream ps = new PrintStream(fos);
			ps.println(); // 개행, 줄바꿈
			ps.println("[ 프린트 보조 스트림 ]");
			ps.print(1);
			ps.print("등 입니다.");
			ps.println("마치 콘솔에 출력하듯이");
			...
			FileWriter fw = new FileWriter(file,true); // true : 이어쓰기 
			// PrintWriter (문자기반)
			PrintWriter writer = new PrintWriter(fw,true); // true : autoFlush
			writer.println("[ PrintWriter ]");
			writer.println("사용 방법은 동일");
			writer.printf("내 이름은 %s 입니다. %n", "카리나");
