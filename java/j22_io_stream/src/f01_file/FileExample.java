...
public class FileExample {
	...
	public static void main(String[] args) {
		...
		// file exists - 존재한다.
		// file 이 저장한 위치에 디렉토리나 파일이 존재하는지 여부를 판단
		...
		System.out.println(file.getAbsolutePath()); // 정확한 경로를 확인
		//C:\Temp\dir\aaa
		
		System.out.println(file.getPath());
		
		// 존재하지 않을때 false를 true로 반환
		if(!file.exists()) {
			// file 객체에 저장된 경로 디렉토리나 파일 정보가 존재하지 않음.
			
			// 지정된 경로에 있는 디렉토리 중 마지막에 정의된 디렉토리 생성
			// C:\Temp\dir\aaa
			// 중간에 지정된 폴더가 존재하지 않으면 생성 불가
		...
		// 지정된 경로에 마지막 디렉토리 또는 파일 정볼르 삭제 후 실해 여부 반환
