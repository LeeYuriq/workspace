...
public class ClientExample {
	...
	public void startClient() {
		try {
			socket = new Socket();
			// 서버에서 연결이 수락 될 때 까지 blocking - 대기
			socket.connect(new InetSocketAddress("10.100.205.157",5002));
			System.out.println("연결 완료");
			System.out.println(socket.getRemoteSocketAddress()); // server
			
			// 서버에서 전달되는 메시지를 수진 대기 및 수신 처리
			receive();
			
			// 콘솔을 통해서 서버에 출력 메세지를 입력 받아 메세지 출력
			while(true) {
